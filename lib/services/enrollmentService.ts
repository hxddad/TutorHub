...
// FR4 + NFR2 + NFR4 - enroll a student in a course
...
export async function enrollStudent(studentId: string, body: any) {
  ...
  // FR4 - course must exist and be published for enrollment to be allowed
  const course = await courseRepo.findCourseById(courseId);
  if (!course || !course.isPublished) {
    throw { status: 404, message: "Course not found" };
  }

  // FR4 - return existing enrollment rather than erroring if already enrolled
  const existing = await enrollmentRepo.findEnrollment(studentId, courseId);
  if (existing) {
    return { enrollment: existing, alreadyEnrolled: true };
  }

  // FR4 - enforce capacity limit if one is set on the course
  if (course.capacity) {
  ...
  // FR4 - create the enrollment using the verified JWT student identity
